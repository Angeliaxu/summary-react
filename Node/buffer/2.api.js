/* 
   Buffer.alloc(size[, fill[, encoding] ]) 返回一个指定长度并被filled填满的Buffer实例，这个方法
   比Buffer.allocUnsafe(size)要满，但是能保证被创建的Buffer实例不会包含旧的并且敏感的数据。
   ...
   Buffer.allocUnsafe(size)、Buffer.allocUnsafeSlow(size)：
        ...
        2.如果Buffer.allocUnsafe(size)的size小于Buffer.poolSize或者等于Buffer.poolSize的一半，
