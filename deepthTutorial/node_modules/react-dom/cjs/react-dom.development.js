...
function enqueueUpdate(fiber, update) {
  ...
  // fiber 是fiberNode
  var alternate = fiber.alternate; // 第一次为null
...
function requestCurrentTimeForUpdate() {
  // executionContext 8， RenderContext 16 ， CommitContext 32
...
function scheduleUpdateOnFiber(fiber, expirationTime) {
  ...
  var priorityLevel = getCurrentPriorityLevel();// 97
...
function unbatchedUpdates(fn, a) {
  var prevExecutionContext = executionContext; // executionContext 0 
  executionContext &= ~BatchedContext; // BatchedContext 1 
  executionContext |= LegacyUnbatchedContext; // LegacyUnbatchedContext 8
 // executionContext: 8 
  try {
    return fn(a);
  } finally {
    executionContext = prevExecutionContext;
    // executionContext: 0
    if (executionContext === NoContext) { // NoContext 0
      // Flush the immediate callbacks that were scheduled during this batch
      flushSyncCallbackQueue();
    }
  }
...
// 在这里，改造了fiberNode里面的fiber.updateQueue
...
function createRootImpl(container, tag, options) {
  ...
  console.log('createRootImpl', root)
